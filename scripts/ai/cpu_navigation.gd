...
static func segment_clear(arena, from: Vector2, to: Vector2, clearance: float = 18.0) -> bool:
	if not arena.fighter_bounds.has_point(to): return false
	for n in range(1,ceili(from.distance_to(to)/8.0)+1):
		if arena.solid(from.move_toward(to,n*8.0),clearance): return false
	return true
...
static func combat_path(arena, start: Vector2, enemy: Vector2, band: Vector2, max_nodes: int = MAX_NODES, clearance: float = 18.0) -> Array:
	...
	while cursor < frontier.size() and cursor < max_nodes:
		...
		if firing_position(arena,point,enemy,band) and (clearance <= 18.0 or segment_clear(arena,point,enemy,clearance)):
			var path: Array = []
		...
		for offset in [Vector2i.RIGHT,Vector2i.DOWN,Vector2i.LEFT,Vector2i.UP]:
			...
			if not segment_clear(arena,point,start+Vector2(next)*CELL,clearance): continue
			previous[next] = cell
