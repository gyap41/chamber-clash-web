...
static func begin(game) -> void:
	...
	if not room_state.has("enemy_ids"):
		...
		room_state.enemy_ids = composition(game.arena,introduced)
	...
	for index in range(positions.size()):
		...
		actor.set_script(Quillback if room_state.enemy_ids[index] == "quillback" else (Lizard if room_state.enemy_ids[index] == "fire_pouch_lizard" else Actor))
