...
ゲーム性の拡張は [ローグライク×対人 実装計画](ROGUELIKE_PVP_PLAN.md) で管理します。P0〜P4の持ち越し・装備交換・成長報酬・CPU対応を実装しました。P3シナジーとP4先行入力・終盤火力・危険弾表示も実装済みです。次はP4の実プレイ評価を行います。以下の既存項目と区別し、未実装のルールは現行仕様へ混在させません。
...
実装済み：独立した試合状態、シード付き抽選とローカル計測、3〜6装備枠・8個所持庫、同数の無料成長報酬、主力継承、仮装備の明示取得、引き分け再戦、CPU成長、統合準備画面。既存の20武器・18レリックの効果を継続。

検証状況とログは [TESTING.md](../development/TESTING.md)。P5のコードは未着手。人間同士の比較試合、20〜30試合の観察、準備時間・終盤ダメージ・負荷の評価は引き続き必要。
