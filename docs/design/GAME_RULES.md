...
- 出現直後の取得待ちを経て、武器/レリックはF/Hで宝箱の開封を開始します。範囲内に既定1.5秒留まると取得し、離脱/死亡で中断、他者は開封中の横取り不可。CPUも同じ開封待ちと収納条件を使い、満杯・所持済みの武器を狙いません。弾薬箱は接触で補給します。武器は携行数と無関係に控えの空きを確認して収納します。戦闘中の武器置換はありません。
...
開始時の固定フェザー配置はありません。35秒後に、SUPPORTED（現在35種）から抽選したレリックを中央付近へ共有の宝箱1個として補給します。再補給間隔は90秒なので標準ラウンドでは一度のみ。F/Hで宝箱を開封し、1人1ラウンド1個まで仮装備できます。戦闘中の取得はplayerで判定し、ラウンド開始時の武器を含む配置・開放セルと共通形状を使って空きを確認します。非stackableの既存所持、上限、仮装備取得済みは理由を表示して拒否します。戦闘中の交換はなく、未取得品は42秒で消滅します。

対戦HUDは[C案](BATTLE_UI_C.md)を実装。上部にラウンド・残り時間・スコア・両者HP、下部92pxに自分の武器/弾薬・行動状態を表示します。CPUの武器欄は非表示、ローカル戦は両者分を維持します。レリックは戦闘中に重要状態のアイコンだけを表示し、一覧ボタン/ESCで一時停止して全個体の効果を確認します。空き枠は非表示、仮装備は紫枠、充電は金枠、待ち時間は秒数で区別します。素材はskin.jsonで正式画像へ交換できます。
...
タイトル経由ではCPU対戦を有効化します。キャラクター選択シーンの単体実行ではLocal/CPU切替を使用できます。CPUは常にP2（インデックス1）を担当し、常にP1を狙います。元WebのCPU分岐を基に強化したscripts/ai/cpu_ai.gdが、毎物理フレームPlayer.step()へ渡す移動方向(dx,dy)・照準ジッター・射撃可否を決定します。CPUはキー入力を一切参照しないため、P2用の移動・戦闘キーは無視されます（HUDの武器枠クリックもCPU側は無効）。
