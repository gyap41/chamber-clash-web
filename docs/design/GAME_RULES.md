...
|15 プラネタリウム|8方向（45度間隔）、各0.8ダメージ、発射間隔1.05秒、弾速340、4/12発。各弾の前方±45度にいる敵へ毎秒最大1.8radで追尾、壁で1回反射。逆向きの星まで一斉集中しない|特別投下（S）|
...
- 出現直後の取得待ちを経て、武器/レリックはG/Hで宝箱の開封を開始します。範囲内に既定1.5秒留まると取得し、離脱/死亡で中断、他者は開封中の横取り不可。CPUも同じ開封待ちと収納条件を使い、満杯・所持済みの武器を狙いません。弾薬箱は接触で補給します。武器は携行数と無関係に控えの空きを確認して収納します。戦闘中の武器置換はありません。
...
CPUの主な挙動：距離340px超で接近・200px未満で後退・その間は時間ベースで横移動（キーティング）。所持弾薬・レリック取得条件・武器の未所持と控え8個の空き・S武器かどうかで欲しい補給品を判定し、最寄り（S武器は距離を0.65倍して優先度を上げる）へ寄って取得。危険地帯は実際のダメージ判定（+25px）より広い余裕（+65/+60px）で検知し、アリーナ中心へ退避。敵弾の位置・速度から0.4秒以内の最接近を予測し、当たりそうな弾を横方向へ避ける。遠ざかる弾・壁越しの弾は回避対象から除外し、近接した静止弾も危険として扱う。回避判断間隔は0.35〜0.75秒で、敵弾がない間も回復する。実際の回避クールダウンを守り、壁回避後の移動方向へロールする。120px以内に敵弾が6発以上集中した場合はパルスを自動使用。移動先が壁にぶつかる場合は±90度/180度の候補角度で回避。武器が完全に空なら弾薬のある武器へ自動切替、リロードも自動、64px以内なら近接攻撃、射線が壁で遮られていない（または跳弾・ブーメラン武器で遮られていても構わない）場合のみ射撃。
