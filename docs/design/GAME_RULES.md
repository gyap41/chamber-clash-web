...
3本先取、90秒上限。勝敗後、試合継続時だけ成長段階が進む。グリッドは段階1〜5で
3×2 / 4×2 / 4×3 / 4×4 / 4×4。武器とレリックが同じ領域を使用し、形状が収まることが装備条件。
所持庫は装備中の武器/レリックを合わせて8個。6×6やバッグ拡張は計画段階。
...
同じ候補の同一準備中の二重取得を防止。所持庫が満杯なら取得前に破棄が必要。
...
初期無料報酬・ラウンド間の成長報酬で永続取得します。武器と共通のグリッドに配置し、所持庫は合計最大8個。フィールド補給はそのラウンド限りの仮装備です。
