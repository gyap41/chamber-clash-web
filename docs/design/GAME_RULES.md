...
|帰還バッテリー（P3）|帰還する弾を持ち主が回収すると充電（最大1つ）。武器切替の後、次の1射に追加ダメージ。命中や寿命切れは回収扱いにしません|
...
反響の種・空薬莢の祝福・帰還バッテリー・余熱コンデンサ・残響ホルスター・すり抜け装填（id12〜17）は`run_tests.ps1`のheadlessテスト（`tests/synergies.gd`ほか）で検証済みです。検証時に見つかった不具合と、既存テストスイートに残る（P3とは無関係の）非決定性については[TESTING.md](../development/TESTING.md)を参照してください。
