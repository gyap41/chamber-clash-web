...
敵・ボスの素材制作では[敵素材の制作テンプレート](ENEMY_CREATION_TEMPLATE.md)から始める。必要な方向・歩行コマ・関節パーツ・SEを制作前に洗い出し、生成済み／本編接続／受入を分けて記録する。

ボス制作は[ボス制作テンプレート](BOSS_CREATION_TEMPLATE.md)も必読。攻撃選択・停止対策、素材とSEの対応表、開始・撃破・報酬、検証までをまとめている。

ステージ制作を始める際は、まず[ステージ制作テンプレート](STAGE_CREATION_TEMPLATE.md)を読む。仕様票・必要素材一覧・壁接続の判断・制作手順・完了チェックを共通化している。

ステージ素材を新規制作する前に、[共通制作ガイド](STAGE_ASSET_GUIDE.md)を参照。見下ろし角度、低頭身キャラとの縮尺、テーブルの天板主体の構図、壁接続、透過・影・発光分離と確認手順をまとめている。
