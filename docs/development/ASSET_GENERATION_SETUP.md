...
記載設定はローカル実装の参照用。API変更時は公式仕様を再確認する。過去の生成枠・承認・費用経緯は[整理前の記録](../archive/2026-09-22/markdown-audit/ASSET_GENERATION_SETUP_BEFORE.md)に分離。過去の承認額や回数は新しい生成の許可ではない。
...
初回構築時には音響19件のローカルテストと、BGM20秒WAV/SE MP3のResource認識が成功した。これは当時の結果であり、今回の資料監査で再実行した結果ではない。
