> 整理前の履歴（2026-09-22保存）。本文の「現在」「今回」「承認」は当時の記録であり、現行指示ではない。

# AI素材生成環境 設定・連携ガイド

## 現行画像枠：接続用壁材セット（2026-09-22）

1送信成功、usage換算推定$0.106578、累計78送信。今回の追加生成は終了。原本と実行用コピー、4つのAtlasTexture、抽出manifestを保存済み。

ユーザーの追加生成・ステージテンプレート記録の指示に基づく1シート。縁なし上面・正面石積み・金属地・露出壁端の4区画。参照画像なし、既存API用CLI、gpt-image-2/high/1024×1024/PNG、認証・通信・依存関係は維持。追加1送信、保守予約$1、累計最大78送信、到達前停止上限79。過去の成否不明予約は保持。再送・追加候補なし。プロンプトはdocs/art/production/wall-kit-v2/prompt.txt。以下は過去の生成記録。


## 現行画像枠：低い工房遮蔽物（2026-09-22）

1シート生成成功、usage換算推定$0.106328。累計77送信。今回の追加生成は完了し、再送しない。

ユーザーの石ブロック置換指示に基づく1シート（資材箱・パレット・金床）、参照なし文章のみ。保守予約$1、累計最大77送信、到達前停止上限78。既存gpt-image-2/high/1024×1024/PNGと認証・通信を維持。再送なし。プロンプトはdocs/art/production/workshop-showcase/low-props-final.txt。以下は過去の記録。


## 現行画像枠：見下ろし作業台（2026-09-22）

ユーザーの作業台の視点修正への承認に基づく1送信。参照画像なし、文章のみ。保守予約$1、累計最大76送信、到達前停止上限77。gpt-image-2 / high / 1024×1024 / PNG。API・認証・通信・依存関係の変更なし。再送・追加候補なし。プロンプトはdocs/art/production/workshop-showcase/workbench-overhead-prompt.txt。1送信成功、累計76送信、usage換算推定$0.106310（請求額ではない）。原本保存後に透過化してゲームへ接続。追加生成終了。以下は以前の制作履歴。


## 現行画像枠：工房完成見本

ユーザーの素材生成・配置・照明を進める承認に基づき、壁床1枚と家具1枚の計2送信。参照画像は送らず文章のみ。保守予約$2、累計最大75送信、到達前停止上限76。gpt-image-2 / high / 1024×1024 / PNG、API・認証・通信設定は維持。再送・追加候補なし。プロンプトはdocs/art/production/workshop-showcase。2送信とも成功、累計75送信、今回usage換算推定$0.212610（請求額ではない）。素材抽出・開始部屋への接続と描画確認済み。追加生成は終了。以下は以前の制作履歴。


## 現行画像枠：外周壁素材（2026-09-21）

ユーザー「素材を作成してもよいですよ。APIから」に基づき、外周壁・角・扉枠向けの4区画素材シートを1枚だけ生成して固定2部屋へ接続する。追加1送信、保守予約$1、累計最大73送信。CLI上限74（到達前停止）。過去の成否不明2件の予約を保持し、自動再送・追加候補は行わない。gpt-image-2 / high / 1024×1024 / PNG、既存cover.pngを参照。provider・model・endpoint・認証・通信・依存関係・保存形式は変更しない。

参照画像・送信先・1枚の生成を明示した確認に対するユーザー「生成お願いします。」の承認後、1送信が成功。累計73送信、今回のusage換算推定$0.107649（請求額ではない）。原本はassets/generated/fw-workshop-walls-v1.png、ゲーム用コピーと4区画のAtlasTextureはassets/first-workshop/environment/。ローカル予算検証7件は成功。外周壁の配置・衝突判定への接続は実装済み。今回の追加生成は終了。

以下の「現行」「完了」は以前の制作枠の記録。
...
HTTP400だけでパラメーター不正と断定しない。2026-09-13に音響CLIの固定表示を訂正。
SEのモデル・尺・出力形式・認証・通信設定は変更せず、生成エラー本文の非表示・非保存を維持。
...
- [音響構築結果](../../2026-09-11/AUDIO_SETUP_REPORT.md)、[TLS調査](../../2026-09-11/AUDIO_TLS_DIAGNOSIS.md)、[403調査](../../2026-09-11/AUDIO_403_DIAGNOSIS.md)
...
画像CLIの送信履歴・排他・予算・参照入力は[画像README](IMAGE_CLI_BEFORE.md#始まりの工房画像制作2026-09-12)を参照。
現在の画像制作向け制限：
