> 整理前の履歴。過去の承認枠・費用・「現行」は当時の記録で、新しい生成の指示ではない。
...
ファイル整理後：生成原画・JSON・使用量履歴はassets/generatedに保持。設定画・確認GIF・プロンプトはdocs/artで区分し、画像加工ツールの参照先を更新した。連番キャプチャは不要候補のraw-framesへ保存し、まとめ画像とGIFだけreviewsへ出力する。生成API・保存形式・予算は変更なし。[整理入口](../../../art/README.md)。整理検証は `python -X utf8 tools/verify_art_organization.py`。one-timeのorganize_generated_art.pyは移動済み台帳があれば再実行を拒否する。
...
画像・SE・BGM共通の環境設定と連携変更手順は [AI素材生成環境ガイド](../../../development/ASSET_GENERATION_SETUP.md) を最初に参照してください。
音響CLIの詳細は [音響生成README](../../../../tools/asset_generator/README.md)。本書は画像CLI固有の手順です。
