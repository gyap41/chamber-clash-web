extends SceneTree
const OUT="res://docs/art/reviews/weapon-readability-fix-2026-09-13/"
const RAW="res://.local/weapon-readability-fix-frames/"
var weapon_id := 37
func _initialize() -> void:call_deferred("run")
func run() -> void:
	for arg in OS.get_cmdline_user_args(): weapon_id=int(arg)
	DirAccess.make_dir_recursive_absolute(RAW);DirAccess.make_dir_recursive_absolute(OUT)
	...
	preload("res://tests/helpers/battle.gd").start(game,weapon_id)
	...
	for frame in range(48):
		...
		assert(shot.save_png(RAW+"%02d-%03d.png"%[weapon_id,frame])==OK)
		if frame==30:assert(shot.save_png(OUT+"battle-%02d.png"%weapon_id)==OK)
