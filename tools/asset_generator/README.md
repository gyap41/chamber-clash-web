...
初回環境構築のBGM1個・SE1個は完了済みの履歴。新しい生成では今回の依頼範囲・候補数を確認し、過去の枠を追加生成の許可として扱わない。
...
422はパラメーター、429は利用/同時実行制限、5xxはサービス側を確認する。
400はBad Requestであり、HTTP番号だけではパラメーター不正と断定しない。
2026-09-13に400の固定表示を訂正。本文非表示・非保存、停止・retryなしは維持。
