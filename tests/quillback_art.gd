...
class Board extends Node2D:
	...
	func _draw() -> void:
		...
		for r in range(4):
			...
			draw_string(font,Vector2(10,100+r*85), "Quillback "+["front","back","right","left"][r%4],HORIZONTAL_ALIGNMENT_LEFT,-1,14)
			for c in range(8):
				var view := {"enemy_id":"quillback","alive":true,"angle":direction,"phase":"chase","remaining":.1,
...
func run() -> void:
	...
	assert(root.get_texture().get_image().save_png("res://.local/quillback-review.png") == OK)
	quit()
