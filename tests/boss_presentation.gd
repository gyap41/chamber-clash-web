...
func run() -> void:
	...
	game._physics_process(.01)
	assert(game.phase == "result" and game.players.size() == 1)
	...
	for frame in range(110): game._physics_process(.02)
	await process_frame
	assert(events.count("boss_explosion") == 1 and events.count("win") == 1)
	assert(game.hud.get_node("Root/Outcome").visible)
	assert(get_nodes_in_group("enemy_death_visuals").is_empty())
	game.start_exploration(22)
	assert(game.victory_sound_delay < 0 and not game.sound.boss_engine.playing)
	game.queue_free()
	await process_frame
	print("PASS: boss muzzle registration, recoil, transition/vent cues, engine pause/mute, death audio after retirement, delayed victory and retry cleanup")
	quit()
