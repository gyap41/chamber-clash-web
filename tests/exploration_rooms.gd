...
func run() -> void:
	...
	for crossing in range(20):
		...
		assert(game.arena.get_node("StageBackground").get_child_count() == game.arena.runtime_definition.placements.filter(func(prop): return not prop.floor_decal).size())
	...
	assert(not player.rally_enabled and player.rally_wounds.is_empty())
	assert(player.weapon().clip == 2 and player.weapon().reserve == 7)
	game._physics_process(before.state.reload)
	assert(player.state.reload == 0 and player.weapon().clip == player.definition().mag and player.weapon().reserve == 7)
