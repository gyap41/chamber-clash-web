...
func run() -> void:
	...
	launch(game)
	assert(s.items.size() == 4 and s.items[0].gun == s.items[1].gun)
	assert(s.items[0].position == Vector2(560,125))
	assert(s.put_item("weapon",4,Vector2(560,125)) == null)
	...
	for i in range(100): assert(s.weighted_gun() in game.Weapons.SUPPORTED)
	...
	s.step(5.0)
	assert(s.legendary_spawned and s.items.size() == 8)
	...
	var marker = s.get_node("Spawns/InitialWeapons/Point1")
	marker.position = Vector2(110,220)
	launch(game)
	assert(s.elapsed == 0 and not s.legendary_spawned and s.items.size() == 4)
