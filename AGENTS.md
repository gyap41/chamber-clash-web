...
- 新規ボスの設計・素材制作・実装・レビューでは `docs/art/BOSS_CREATION_TEMPLATE.md` と敵素材の共通テンプレートを読む。ボス別の仕様票・攻撃対応表・素材/SE台帳を制作記録へ残す。
- 攻撃選択、突進や移動失敗後の復帰、開始/段階移行/撃破/報酬、音の同期と停止条件まで確認する。既存ボス固有の数値・材質・攻撃を新ボスの必須条件にしない。
