...
現在の音は `scripts/audio/sound.gd` の合成処理です。音声ファイルを導入した時点で `audio/sfx/` と `audio/music/` を追加します。UI画像は `ui/`、背景は `environments/`、キャラ別の素材が増えたら `characters/<character_id>/` とします。用途が生じる前に空の分類を増やす必要はありません。
